...
type Student struct {
	Model
	Nim 		string 				`gorm:"type:varchar(18);not null" json:"nim" valid:"stringlength(16|16)"`
	...
	Entrance	string 				`gorm:"type:varchar(100);not null" json:"entrance" valid:"required"`
	KRS			[]KRS				`gorm:"foreignKey:StudentID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE;"`
}

type StudentAuth struct {
	Nim 		string 		`gorm:"type:varchar(18);not null" json:"nim" valid:"required,stringlength(16|16),numeric"`
	Password 	string 		`gorm:"type:varchar(255);not null" json:"password" valid:"required,stringlength(8|255),isalphanum"`
}